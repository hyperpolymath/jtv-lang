...
This document tracks all items marked as TODO in the academic
documentation and identifies gaps between the theoretical foundations
...
[width="100%",cols="15%,36%,27%,22%",options="header",]
|===
...
*Impact:* The type system is functional but lacks advanced features for
...
|[line-through]#PROOF-5# |[line-through]#Some Lean proofs use `+sorry+`#
— *RESOLVED 2026-06-02*:
`+grep -rc 'sorry\|admit\|axiom' jtv_proofs/*.lean+` ⇒ 0. The proof
layer is `+sorry+`-free. |— |✅ Done
|===

*Impact:* While core theorems are proven, full verification chain is
incomplete.

____
*Doc-truthing note (2026-06-02):* This section previously overstated
proof incompleteness. The authoritative, continuously-checked status now
lives in
link:../verification/PROOF-CAPABILITY-MATRIX.adoc[`+verification/PROOF-CAPABILITY-MATRIX.adoc+`].
____
...
*No proof in `+jtv_proofs/+` uses `+sorry+`, `+admit+`, or an added
`+axiom+`.* The claims previously listed here are stale:

* `+data_terminates+` (now `+JtvOperational.lean:~341+`) is a _complete_
structural induction over `+DataExpr+` (lit / var / add / neg),
discharged via `+dataStepStar_trans+` and the `+dataStepStar_*+`
congruence lemmas. No `+sorry+`.
* The Control-divergence fact is shown by the fully-proved
`+infinite_loop_steps+`, not a `+sorry+`-d example.

Verification: `+grep -rc 'sorry\|admit\|axiom' jtv_proofs/*.lean+` ⇒ all
zero. See `+verification/PROOF-CAPABILITY-MATRIX.adoc+` for the
...
|`+wasm_compilation_correctness+` |Not created |WASM output matches
semantics
...
Current estimated coverage: - Parser: ~70% - Interpreter: ~60% - Type
checker: ~50% - Purity analyzer: ~40% - Reversible: ~30%
...
|proofs/MATHEMATICAL_FOUNDATIONS.md
...
. ☐ Document parser grammar formally

==== 7.2 Short-term (P1)

[arabic]
. ☐ Implement symbolic simplification
. ☐ Add effect system formalization
. ☐ Complete type inference proofs
...
. *Mechanized proofs* for core properties
. *Clear separation* of Data/Control guarantees
. *Well-cited* with authoritative references

==== 9.2 Areas Needing Strengthening

[arabic]
. *Proof completeness* - Some `+sorry+` usage
. *Implementation verification* - Parser/interpreter not fully verified
. *Advanced type features* - Documented but not implemented
. *Empirical validation* - Fuzzing/testing incomplete

==== 9.3 Recommended Path Forward

[arabic]
. Complete all Lean proofs
. Run fuzzing campaign
. Seek peer review
. Submit to programming languages venue
...
*Note:* This document should be updated as gaps are addressed. Each
completed item should be marked with ✓ and dated.
