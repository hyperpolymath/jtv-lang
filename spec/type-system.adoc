...
Widening is always to a strictly more general type. No implicit
narrowing.
...
[arabic]
. *Harvard safety:* Data expressions cannot perform I/O or loop —
guaranteed by type separation.
. *Data totality:* All data expressions terminate (no recursion, no
loops, addition-only).
. *Numeric coercion soundness:* Widening preserves mathematical meaning
(Int ⊂ Float ⊂ Complex).
. *Purity monotonicity:* Calling a less-pure function demotes the
caller’s purity level.
. *Reverse block safety:* Only invertible statements allowed inside
reverse blocks.
. *Code injection impossibility:* Harvard separation makes it
grammatically impossible for data to execute as control — this is a
